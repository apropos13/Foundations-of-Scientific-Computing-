...
!!$  real, dimension(:,:), allocatable :: dummyVar
  ...
  if ((nIter < maxIter) .and. (residual < threshold)) then
     ...
     print *, 'Initial Guess value = ', xInit !line added to show initial guess
     if(ftnType == 1) then 
      print *, 'Function type is (1) hence f(x) = x + exp(x) + 10/(1+x^2) - 5' 
    else if (ftnType == 2) then
      print *, 'Function type is (2) hence f(x) = (x - 1.) * (x - 2.)' 
    else if (ftnType == 3) then 
      print *, 'Function type is (3) hence f(x) = exp(2. * x + 3) - exp(x)'
    else if (ftnType == 4) then 
      print *, 'Function type is (4) hence f(x) = x^2 + 4x + 4'
    endif
